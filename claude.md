...
- **SSRF guard**: Any server-side `fetch()` whose URL is derived from user input (entity URL, scraped `<link>` href, OG image, etc.) MUST go through `safeFetch` from [src/lib/url-guard.ts](src/lib/url-guard.ts). It resolves the hostname via DNS, refuses any address in loopback / private / link-local / unique-local / reserved / multicast / benchmarking / carrier-grade-NAT / unspecified / broadcast ranges (uses `ipaddr.js`), follows redirects manually with re-validation on each hop, and aborts after a total timeout. Used by `src/lib/logoScraper.ts` and `src/lib/scraper.ts`. Constant hosts (Google S2, DDG, OpenAI, Anthropic, Mistral, Google AI, Mollie API) are exempt — they're not user-controlled.
- **Log injection / format-string**: Don't interpolate user-controlled values into the format string of `console.error/log/warn`. Pass them as separate arguments instead: `console.error('Operation failed:', userValue, err)`. CodeQL flags template-literal interpolation as format-string findings; the separate-arg pattern is recognized as safe and avoids any CRLF log-forging risk.
...
- **Health endpoint**: `/api/health` — unauthenticated, returns 200 + version + timestamp if DB is reachable, 503 if not. Used by external monitors.
- **Versioning**: Single source of truth is `package.json.version`. `next.config.js` reads it at build time and (a) injects it into the client bundle as `NEXT_PUBLIC_APP_VERSION` and (b) sets an `X-App-Version` response header on every route. Consumers: sidebar footer ([src/components/Sidebar/SidebarFooter.tsx](src/components/Sidebar/SidebarFooter.tsx)), `/api/health` JSON, all three Sentry configs (`release: process.env.NEXT_PUBLIC_APP_VERSION`). **To cut a release**: bump `package.json.version`, add a CHANGELOG entry, commit, push — that's it. No second hardcoded edit anywhere.
- **Error tracking (Sentry)**: `@sentry/nextjs` v10 with tunnel at `/monitoring`. Config files: `instrumentation-client.ts` (browser), `sentry.server.config.ts` (Node), `sentry.edge.config.ts` (Edge), `instrumentation.ts` (server hook). Global error boundary at `src/app/global-error.tsx`. Only enabled in production (`enabled: process.env.NODE_ENV === "production"`). Releases tagged with `package.json.version` via `release:` in each Sentry config — errors are grouped per release in the Sentry UI. Session Replay 5%, tracing 10%. Source map upload disabled until `SENTRY_AUTH_TOKEN` is set in Coolify. CSP `connect-src` includes `*.ingest.de.sentry.io`.
...
### `cookies().set()` is only legal in Server Actions or Route Handlers

- **Issue**: Calling `cookies().set(...)` (or `delete`) from a Server Component (any `page.tsx` / `layout.tsx` render path) throws at runtime: *"Cookies can only be modified in a Server Action or Route Handler"*. Reads are fine — only writes are restricted. The error surfaces in prod via Sentry with `mechanism: auto.function.nextjs.on_request_error` and crashes the page.
- **Solution**: If the URL is a GET that has side-effects (e.g. an OAuth callback, an invite-accept link, a "set current X" toggle), make it a Route Handler — `route.ts` in the same folder, `export async function GET(...)`. Route Handlers can `cookies().set()` freely and can return `NextResponse.redirect(...)`. If the side-effect is triggered from a form/button, use a Server Action (`'use server'`) instead.
- **Don't** try to set cookies from `page.tsx` even via a helper — the rule is about the calling render context, not the file location.
- **Example fix**: [src/app/invite/[token]/accept/route.ts](src/app/invite/[token]/accept/route.ts) — previously a Server Component that wrote the `current-space-id` cookie after accepting an invite. Moved to a Route Handler with the same URL; the parent `/invite/[token]` page's `callbackURL` continues to work unchanged.
...
### Deploy fails at "exporting layers" with `archive/tar: sockets not supported`

- **Symptom**: Coolify deploy fails with exit 255 right after `#16 exporting layers`. Docker daemon log shows `Can't add file /var/lib/docker/overlay2/.../diff/nix/var/nix/gc-socket/socket to tar: archive/tar: sockets not supported`. Build itself succeeded (`✓ Compiled successfully`); only the image-export tar step fails.
- **Cause**: Nixpacks stage 4 runs `nix-env … && nix-collect-garbage -d`, which sometimes leaves the Nix daemon's Unix-domain socket at `/nix/var/nix/gc-socket/socket`. When BuildKit tars the final image, Go's `archive/tar` refuses to serialize sockets. Intermittent — depends on whether the nix stage is cache-hit and on `nix-collect-garbage` timing. Not caused by app code.
- **Fix (in order)**:
  1. Retry the deploy. Often passes on the next run (different nix-stage cache state).
  2. If it re-fires: `docker builder prune -a -f` on the host, then redeploy. Forces stage 4 to re-run clean.
  3. If chronic: swap Coolify build pack from Nixpacks → Dockerfile. The repo's `Dockerfile` is unaudited for the worker `node_modules` and Prisma OpenSSL on alpine — needs a real pass before flipping.
- **First seen**: 2026-05-19. Single occurrence so far; resolved by redeploy.
