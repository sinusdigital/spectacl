...
async function _getCompetitorVisibilityHistory(
    ...
    const mentions = await prisma.$queryRawUnsafe(query, ...params);

    // Group by Competitor
    const competitorsMap = new Map<string, { id: string; name: string; history: any[] }>();
