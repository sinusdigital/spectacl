...
import { Flex, Box, Text, Heading, Button, Badge, Select, Callout, Table } from '@radix-ui/themes';
                ...
                <Table.Cell>
                  {member.userId !== currentUserId && (
                    <Flex gap="2" justify="end">
                      <Dialog.Root open={showRoleModal && selectedMember?.userId === member.userId} onOpenChange={(open) => {
                        if (!open) { setShowRoleModal(false); setSelectedMember(null); }
                      }}>
                        <Dialog.Trigger asChild>
                          <Button variant="ghost" size="1" disabled={member.role === 'OWNER'} onClick={() => {
                        ...
                        <Dialog.Portal>
                          ...
                          <Dialog.Content className="dialog-content">
                            <Flex direction="column" gap="4">
                              <Heading size="5">Change Member Role</Heading>
                              <Text size="2" color="gray">
                                Change role for <strong>{selectedMember?.name || selectedMember?.email}</strong>
                              </Text>
                              <Box>
                                <Text as="label" size="2" weight="bold" mb="1" style={{ display: 'block' }}>New Role</Text>
                              ...
                              <Flex gap="3" justify="end">
                                ...
                                <Button onClick={handleChangeRole} disabled={processing || newRole === selectedMember?.role}>
                      ...
                      <Dialog.Root open={showRemoveModal && selectedMember?.userId === member.userId} onOpenChange={(open) => {
                        if (!open) { setShowRemoveModal(false); setSelectedMember(null); }
                      }}>
                        <Dialog.Trigger asChild>
                          <Button variant="ghost" color="red" size="1" disabled={member.role === 'OWNER'} onClick={() => {
                            setSelectedMember(member);
                        ...
                        <Dialog.Portal>
                          ...
                          <Dialog.Content className="dialog-content">
                            <Flex direction="column" gap="4">
                              <Heading size="5">Remove Member?</Heading>
                              <Text size="2" color="gray">
                                Are you sure you want to remove <strong>{selectedMember?.name || selectedMember?.email}</strong> from this space? They will lose access to all entities and data.
                              </Text>
                              <Flex gap="3" justify="end">
                                <Dialog.Close asChild>
                                  <Button variant="soft" color="gray">Cancel</Button>
