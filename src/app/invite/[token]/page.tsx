...
export default function InvitePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/invitations/${token}`)
      ...
      .catch(err => {
        // If the user is already signed in but the invitation is no longer
        // fetchable (consumed by a previous attempt, expired, etc.), don't dead-end
        // them on an "Invalid Invitation" screen — they may already be a member.
        // Send them to /auth/callback which routes them to their workspace or
        // onboarding based on actual membership state.
        if (session?.user) {
  ...
  }, [token, sessionLoading, session]);

  // Absolute callbackURL anchored to the browser's actual origin. Defends against
  // misconfigured BETTER_AUTH_URL — if that env is wrong server-side, a relative
  // callbackURL gets resolved against the wrong base and the post-verify redirect
  // ends up on localhost.
  const callbackURL =
